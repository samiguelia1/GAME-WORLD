...
return(
   <div>
           ...
           <img src="public/logo.svg" className=" sm:mx-10 mx-2   h-[45px] cursor-pointer " alt="" />
                    <div className="w-screen "><i onClick={SerachForGame} className=" cursor-pointer fas fa-search text-white relative left-8  "></i><input onChange={setgamename} onKeyDown={SerachForGame} type="text" placeholder="Searh for a game..." className="input "/></div>
            </nav>
          ...
          {
           isLoading ? <div className="flex w-screen h-[60vh] justify-center items-center"><img src="public/loading.svg" className="lg:h-[40%] h-[20%] sm:30% " alt="" /></div>

                     :<ul  className="sm:flex flex-wrap grid sm:flex-row ">
                              { games.map((game,index)=>(
                        
                        <div key={game?.id}  className="  flex flex-col my-5 overflow-hidden text-white w-1/1 sm:w-1/4 lg:w-1/6 hover:scale-110 lg:ml-8 sm:ml-15 rounded-lg pb-2 cursor-pointer duration-300 hover:bg-[#515050] bg-[#2e2e2e] ">                                 
                                    ...
                                    <i className="fas fa-download ml-2 mr-1 text-[13px]"></i><span className=" text-[12px] font-semibold font-mono " > {game?.added_by_status?.playing}</span>
                                    <i className="fas fa-star ml-2 text-[13px]  text-amber-300"></i><span className=" text-[12px] font-semibold font-mono mx-1 " >{Math.floor(game?.rating*10)/10}</span>
                         <p className="text-[12px] font-semibold font-mono mx-1"><span className=" ml-2 mr-1 fas fa-clock"></span>{game?.released?.split("-")[0]}</p>
                                    </div>
                        </div>
                  ))}</ul>
